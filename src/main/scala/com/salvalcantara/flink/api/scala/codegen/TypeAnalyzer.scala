...
trait TypeAnalyzer[C <: Context] {
  ...
  private class UDTAnalyzerInstance {
    ...
    private def analyzeArray(id: Int, tpe: Type, elemTpe: Type): UDTDescriptor = analyze(elemTpe) match {
      case UnsupportedDescriptor(_, _, errs) => UnsupportedDescriptor(id, tpe, errs)
      case desc                              => ArrayDescriptor(id, tpe, desc)
    }

    private def analyzeTraversable(id: Int, tpe: Type, elemTpe: Type): UDTDescriptor = analyze(elemTpe) match {
      case UnsupportedDescriptor(_, _, errs) => UnsupportedDescriptor(id, tpe, errs)
    ...
    private def analyzeTry(id: Int, tpe: Type, elemTpe: Type): UDTDescriptor = analyze(elemTpe) match {
      case UnsupportedDescriptor(_, _, errs) => UnsupportedDescriptor(id, tpe, errs)
      case elemDesc                          => TryDescriptor(id, tpe, elemDesc)
    }

    private def analyzeOption(id: Int, tpe: Type, elemTpe: Type): UDTDescriptor = analyze(elemTpe) match {
      case UnsupportedDescriptor(_, _, errs) => UnsupportedDescriptor(id, tpe, errs)
    ...
    private def analyzePojo(id: Int, tpe: Type): UDTDescriptor = {
      ...
      val invalidFields = fields.filterNot { f =>
        f.isPublic ||
      ...
      val hasZeroCtor = tpe.declarations.exists {
        case m: MethodSymbol if m.isConstructor && m.paramss.length == 1 && m.paramss(0).length == 0 =>
          true
        case _ => false
      }
      ...
      tpe.baseClasses.exists(bc => !(bc == tpe.typeSymbol) && bc.asClass.isCaseClass) match {
        ...
        case false =>
          ...
          ctors match {
            ...
            case ctor :: Nil =>
              val caseFields = ctor.paramss.flatten.map { sym =>
              ...
              val fields = caseFields.map { case (fgetter, fsetter, fTpe) =>
              ...
              fields.filter(_.desc.isInstanceOf[UnsupportedDescriptor]) match {
                case errs @ _ :: _ =>
                  val msgs = errs.flatMap { f =>
                    (f: @unchecked) match {
                      case FieldDescriptor(fName, _, _, _, UnsupportedDescriptor(_, fTpe, errors)) =>
...
//            .find(t => t.erasure =:= typeOf[TraversableOnce[_]].erasure)
          ...
          traversable match {
            case TypeRef(_, _, elemTpe :: tpe :: Nil) =>
    ...
    private object ValueType {
      def unapply(tpe: Type): Boolean =
        tpe.typeSymbol.asClass.baseClasses.exists { s =>
          s.fullName == "org.apache.flink.types.Value"
        }
    }
    ...
    private object WritableType {
      def unapply(tpe: Type): Boolean =
        tpe.typeSymbol.asClass.baseClasses.exists { s =>
          s.fullName == "org.apache.hadoop.io.Writable"
        }
    }
  ...
  lazy val boxedPrimitives = {
    ...
    def getBoxInfo(prim: Symbol, primName: String, boxName: String) = {
      ...
      val box = { t: Tree =>
        Apply(
          Select(Select(Ident(newTermName("scala")), newTermName("Predef")), newTermName(primName + "2" + boxName)),
          List(t)
        )
      }
      val unbox = { t: Tree =>
        Apply(
          Select(Select(Ident(newTermName("scala")), newTermName("Predef")), newTermName(boxName + "2" + primName)),
          List(t)
        )
      }
