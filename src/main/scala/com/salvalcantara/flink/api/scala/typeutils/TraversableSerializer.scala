...
class TraversableSerializer[T <: TraversableOnce[E], E](var elementSerializer: TypeSerializer[E], var cbfCode: String)
...
object TraversableSerializer {
  ...
  private val CACHE: Cache[Key, Factory[_, _]] = CacheBuilder
    .newBuilder()
    .weakValues()
    .maximumSize(128)
    .build()
  ...
  case class Key(classLoaderHash: Int, classLoaderRef: WeakReference[ClassLoader], cbfCode: String) {
  ...
  private case class LazyRuntimeCompiler[T, E](classLoader: ClassLoader, code: String) extends Callable[Factory[E, T]] {

    override def call(): Factory[E, T] = compileCbfInternal(classLoader, code)

    private def compileCbfInternal(classLoader: ClassLoader, code: String): Factory[E, T] = {
