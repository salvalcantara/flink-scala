 ...
 */
package com.salvalcantara.flink.api.scala.typeutils
...
/**
 * This class gives access to the type information of the most common Scala
 * types for which Flink has built-in serializers and comparators.
 *
 * This class contains types of [[org.apache.flink.api.common.typeinfo.Types]]
 * and adds types for Scala specific classes (such as [[Unit]] or case classes).
 *
 * In many cases, Flink tries to analyze generic signatures of functions to
 * determine return types automatically. This class is intended for cases where
 * type information has to be supplied manually or cases where automatic type
 * inference results in an inefficient type.
 *
 * Scala macros allow to determine type information of classes and type
 * parameters. You can use [[Types.of]] to let type information be determined
 * automatically.
 */
@PublicEvolving
object Types {
   ...
   * [[org.apache.flink.api.common.typeinfo.TypeHint]] but does not require to
   * implement anonymous classes.
   *
   * If the class could not be analyzed by the Scala type analyzer, the Java
   * analyzer will be used.
   *
   * Example use:
   *
   * `Types.of[(Int, String, String)]` for Scala tuples `Types.of[Unit]` for
  ...
  /**
   * Returns type information for Scala [[Nothing]]. Does not support a null
   * value.
   */
  ...
  /**
   * Returns type information for [[String]] and [[java.lang.String]]. Supports
   * a null value.
   */
  ...
  /**
   * Returns type information for Java [[java.math.BigDecimal]]. Supports a null
   * value.
   *
  ...
  /**
   * Returns type information for Java [[java.math.BigInteger]]. Supports a null
   * value.
   *
  ...
  /**
   * Returns type information for [[java.time.LocalDateTime]]. Supports a null
   * value.
   */
  ...
  /**
   * Returns type information for [[org.apache.flink.types.Row]] with fields of
   * the given types. A row itself must not be null.
   *
   * A row is a fixed-length, null-aware composite type for storing multiple
   * values in a deterministic field order. Every field can be null regardless
   * of the field's type. The type of row fields cannot be automatically
   * inferred; therefore, it is required to provide type information whenever a
   * row is used.
   *
   * <p>The schema of rows can have up to <code>Integer.MAX_VALUE</code> fields,
   * however, all row instances must strictly adhere to the schema defined by
   * the type info.
   *
   * This method generates type information with fields of the given types; the
   * fields have the default names (f0, f1, f2 ..).
   *
   * @param types
  ...
  /**
   * Returns type information for [[org.apache.flink.types.Row]] with fields of
   * the given types and with given names. A row must not be null.
   *
   * A row is a variable-length, null-aware composite type for storing multiple
   * values in a deterministic field order. Every field can be null independent
   * of the field's type. The type of row fields cannot be automatically
   * inferred; therefore, it is required to provide type information whenever a
   * row is used.
   *
   ...
   *
   * A POJO class is public and standalone (no non-static inner class). It has a
   * public no-argument constructor. All non-static, non-transient fields in the
   * class (and all superclasses) are either public (and non-final) or have a
   * public getter and a setter method that follows the Java beans naming
   * conventions for getters and setters.
   *
   * A POJO is a fixed-length, null-aware composite type with non-deterministic
   * field order. Every field can be null independent of the field's type.
   *
   * The generic types for all fields of the POJO can be defined in a hierarchy
   * of subclasses.
   *
   * If Flink's type analyzer is unable to extract a valid POJO type information
   * with type information for all fields, an
   * [[org.apache.flink.api.common.functions.InvalidTypesException}]] is thrown.
   * Alternatively, you can use [[Types.POJO(Class, Map)]] to specify all fields
   * manually.
   *
   * @param pojoClass
  ...
  /**
   * Returns type information for a POJO (Plain Old Java Object) and allows to
   ...
   * automatically, we recommend to use [[Types.POJO(Class)]].
   *
   * @param pojoClass
   *   POJO class
   * @param fields
   *   map of fields that map a name to type information. The map key is the
   *   name of the field and the value is its type.
   */
  def POJO[T](pojoClass: Class[T], fields: Map[String, TypeInformation[_]]): TypeInformation[T] =
    JTypes.POJO(pojoClass, fields.asJava)

  /**
   * Returns generic type information for any Scala/Java object. The
   * serialization logic will use the general purpose serializer Kryo.
   *
   * Generic types are black-boxes for Flink, but allow any object and null
   * values in fields.
   *
   * By default, serialization of this type is not very efficient. Please read
   * the documentation about how to improve efficiency (namely by
   ...
   *
   * A Scala case class is a fixed-length composite type for storing multiple
   * values in a deterministic field order. Fields of a case class are typed.
   * Case classes and tuples are the most efficient composite type; therefore,
   * they do not not support null-valued fields unless the type of the field
   ...
   *
   * A Scala tuple is a fixed-length composite type for storing multiple values
   * in a deterministic field order. Fields of a tuple are typed. Tuples are the
   * most efficient composite type; therefore, they do not not support
   * null-valued fields unless the type of the field supports nullability.
   *
  ...
  /**
   * Returns type information for Scala/Java arrays of primitive type (such as
   * `Array[Byte]`). The array and its elements do not support null values.
   *
   * @param elementType
  ...
  /**
   * Returns type information for Scala/Java arrays of object types (such as
   * `Array[String]`, `Array[java.lang.Integer]`). The array itself must not be
   * null. Null values for elements are supported.
   *
   * @param elementType
  ...
  /**
   * Returns type information for Scala [[Either]] type. Null values are not
   * supported.
   *
  ...
  /**
   * Returns type information for Scala [[Option]] type. Null values are not
   * supported.
   *
  ...
  /**
   * Returns type information for Scala [[Try]] type. Null values are not
   * supported.
   *
  ...
  /**
   * Returns type information for Scala enumerations. Null values are not
   * supported.
   *
   * @param enum
   *   enumeration object
  ...
  /**
   * Returns type information for Scala collections that implement
   * [[Traversable]]. Null values are not supported.
   */
