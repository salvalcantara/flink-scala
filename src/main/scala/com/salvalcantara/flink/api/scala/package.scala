 ...
 */
package com.salvalcantara.flink.api

import com.salvalcantara.flink.api.scala.typeutils.{
  CaseClassTypeInfo,
 ...
 * the starting-point of any Flink program. It can be used to read from local
 * files, HDFS, or other sources. [[org.apache.flink.api.scala.DataSet]] is the
 * main abstraction of data in Flink. It provides operations that create new
 * DataSets via transformations. [[org.apache.flink.api.scala.GroupedDataSet]]
 * provides operations on grouped data that results from
 * [[org.apache.flink.api.scala.DataSet.groupBy()]].
 *
 * Use
 * [[org.apache.flink.api.scala.ExecutionEnvironment.getExecutionEnvironment]]
 * to obtain an execution environment. This will either create a local
 * environment or a remote environment, depending on the context where your
...
package object scala {
  ...
  def explicitFirst[T](funcOrInputFormat: AnyRef, typeInfo: TypeInformation[T]): TypeInformation[T] =
    funcOrInputFormat match {
  ...
  /**
   * The definition of the class [[Tuple2CaseClassSerializer]] is not visible
   * from Java, because it is defined within a scala package object, and we need
   * it in [[Tuple2CaseClassSerializerSnapshot]] so we need to expose it as a
   * subtype, via this method.
   */
  @Internal()
   ...
   * The definition of the class [[Tuple2CaseClassSerializer]] is not visible
   * from Java because it is defined within a scala package object, and we need
   * to be able to create new instances of it from
   * [[Tuple2CaseClassSerializerSnapshot]] so we need to expose it via this
   * method.
   */
  @Internal()
  ...
  def createTuple2TypeInformation[T1, T2](t1: TypeInformation[T1], t2: TypeInformation[T2]): TypeInformation[(T1, T2)] =
    new CaseClassTypeInfo[(T1, T2)](classOf[(T1, T2)], Array(t1, t2), Seq(t1, t2), Array("_1", "_2")) {
