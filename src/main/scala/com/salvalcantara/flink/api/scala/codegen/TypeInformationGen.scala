 ...
 */
package com.salvalcantara.flink.api.scala.codegen
...
@Internal
trait TypeInformationGen[C <: Context] {
  ...
  def mkTypeInfoFromFactory[T: c.WeakTypeTag](desc: FactoryTypeDescriptor): c.Expr[TypeInformation[T]] = {
  ...
  def mkCaseClassTypeInfo[T <: Product: c.WeakTypeTag](desc: CaseClassDescriptor): c.Expr[TypeInformation[T]] = {
    val tpeClazz = c.Expr[Class[T]](Literal(Constant(desc.tpe)))
  ...
  def mkEitherTypeInfo[T: c.WeakTypeTag](desc: EitherDescriptor): c.Expr[TypeInformation[T]] = {
    ...
    val result = q"""
      import com.salvalcantara.flink.api.scala.typeutils.EitherTypeInfo
  ...
  def mkEnumValueTypeInfo[T: c.WeakTypeTag](d: EnumValueDescriptor): c.Expr[TypeInformation[T]] = {
    ...
    val result = q"""
      import com.salvalcantara.flink.api.scala.typeutils.EnumValueTypeInfo
  ...
  def mkTryTypeInfo[T: c.WeakTypeTag](desc: TryDescriptor): c.Expr[TypeInformation[T]] = {
    ...
    val result = q"""
      import com.salvalcantara.flink.api.scala.typeutils.TryTypeInfo
  ...
  def mkOptionTypeInfo[T: c.WeakTypeTag](desc: OptionDescriptor): c.Expr[TypeInformation[T]] = {
    ...
    val result = q"""
      import com.salvalcantara.flink.api.scala.typeutils.OptionTypeInfo
  ...
  def mkTraversableTypeInfo[T: c.WeakTypeTag](desc: TraversableDescriptor): c.Expr[TypeInformation[T]] = {
    ...
    val cbf       = q"scala.collection.Factory[$elemTpe, $traversableTpe]"
    ...
    val result = q"""
      ...
      import com.salvalcantara.flink.api.scala.typeutils.TraversableTypeInfo
      import com.salvalcantara.flink.api.scala.typeutils.TraversableSerializer
      import org.apache.flink.api.common.ExecutionConfig
  ...
  def mkTypeParameter[T: c.WeakTypeTag](typeParameter: TypeParameterDescriptor): c.Expr[TypeInformation[T]] = {
    ...
    if (result.isEmpty) {
      c.error(c.enclosingPosition, s"could not find implicit value of type TypeInformation[${typeParameter.tpe}].")
    }
